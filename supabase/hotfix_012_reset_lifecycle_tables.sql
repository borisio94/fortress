-- ═══════════════════════════════════════════════════════════════════════════
-- Fortress — Hotfix 012 : Ajouter les tables cycle de vie au reset_shop_data
--
-- Le RPC reset_shop_data ne supprimait pas les tables ajoutées au hotfix 010.
-- Ce hotfix met à jour la fonction pour inclure :
--   suppliers, incidents, stock_movements, receptions, reception_items,
--   purchase_orders, purchase_order_items, stock_arrivals
--
...
BEGIN
  ...
  -- Marques et unités (oubliées dans les versions précédentes)
  DELETE FROM brands WHERE shop_id::text = v_sid;
  DELETE FROM units  WHERE shop_id::text = v_sid;

  -- Tables cycle de vie produit (hotfix 010+011)
  DELETE FROM stock_arrivals  WHERE shop_id::text = v_sid;
  DELETE FROM stock_movements WHERE shop_id::text = v_sid;
  DELETE FROM incidents       WHERE shop_id::text = v_sid;
  DELETE FROM reception_items WHERE reception_id IN (
    SELECT id FROM receptions WHERE shop_id::text = v_sid);
  DELETE FROM receptions      WHERE shop_id::text = v_sid;
  DELETE FROM purchase_order_items WHERE order_id IN (
    SELECT id FROM purchase_orders WHERE shop_id::text = v_sid);
  DELETE FROM purchase_orders WHERE shop_id::text = v_sid;
  DELETE FROM suppliers       WHERE shop_id::text = v_sid;
