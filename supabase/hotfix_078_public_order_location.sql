-- ════════════════════════════════════════════════════════════════════════════
-- hotfix_078_public_order_location.sql
--
-- place_public_order v4 : ajoute `p_location_id` (l'emplacement de stock
-- choisi par le marchand AU MOMENT DU PARTAGE du catalogue). Stocké dans
-- `orders.delivery_location_id` (colonne existante, hotfix_015) → la
-- commande web est rattachée au bon emplacement et le décrément de stock
-- côté marchand frappe ce périmètre, comme une commande livraison normale.
--
-- 100 % rétro-compatible : `p_location_id` est optionnel (DEFAULT NULL).
-- NULL → delivery_location_id reste NULL → comportement v3 inchangé
-- (commande rattachée à la boutique seule). Aucune régression possible
-- pour les liens déjà en circulation (sans le paramètre `loc`).
--
-- DROP+CREATE de la signature v3 pour éviter l'ambiguïté de résolution
-- avec arguments nommés. Idempotent.
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- ── Validation emplacement (défensive) ────────────────────────────────
  -- On n'écrit delivery_location_id QUE si l'emplacement existe vraiment
  -- (un id corrompu dans un vieux lien ne doit pas casser la commande :
  -- on retombe alors silencieusement sur NULL = boutique).
...
-- Vérif : la commande web doit porter delivery_location_id =
--   l'emplacement passé. SELECT id, shop_id, delivery_location_id, source
--   FROM orders WHERE source='web' ORDER BY created_at DESC LIMIT 5;
